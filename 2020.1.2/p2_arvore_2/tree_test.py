...
class TestNode(unittest.TestCase):
    ...
    def test_position_node(self):
        ...
        for i in range(3, 7):
            self.assertEqual(self.arr_tests[i].position_node(8), arr_expected[i - 3],
                             f"Erro na árvore de teste posição {i - 3}")

    def test_is_balanced(self):
        arr_expected = [0, 1, 0, 1]

        for i in range(3, 7):
            self.assertEqual(self.arr_tests[i].is_balanced(), arr_expected[i - 3],
                             f"Erro na árvore de teste posição {i - 3}")

    def test_to_balanced_tree(self):

        for i in range(3, 7):
            self.assertEqual(self.arr_tests[i].to_balanced_tree().is_balanced(), 1,
                             f"Erro na árvore de teste posição {i - 3}")
